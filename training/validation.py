...
VAL_IMG_SIZE = config.getint('Training', 'train_img_size')
...
def validation_loop(net):
    ...
    imgs_data_loader = DataLoader(image_dataset, batch_size=BATCH_SIZE, 
                                  ...
                                  collate_fn=(lambda batch: collate_func(batch,
                                                                         FIXED_SIZE_IMGS,
                                                                         resize_batch=VAL_IMG_SIZE,
                                                                         diff_sketch=False)))

    val_metrics = {'PSNR':[], 'SSIM':[], 'FID':[]}
    ...
    for batch_ndx, img_batch in enumerate(imgs_data_loader, start=1):
        ...
        val_metrics['PSNR'].append(psnr_index.item())
        val_metrics['SSIM'].append(ssim_index.item())
        val_metrics['FID'].append(fid.item())
